...
class RegisterIO implements IIOProvider {
  ...
  private void getCurrentData() {
    ...
    if (io_provider.read(first_address, curr_data)) {
      ...
      notify_sink.setBoardState(
          board_state, update_board_status); // Board status already updated previously above
  ...
  public void zeroYaw() {
    io_provider.write(
        IMURegisters.NAVX_REG_INTEGRATION_CTL, AHRSProtocol.NAVX_INTEGRATION_CTL_RESET_YAW);
  ...
  public void zeroDisplacement() {
    io_provider.write(
        IMURegisters.NAVX_REG_INTEGRATION_CTL,
        (byte)
