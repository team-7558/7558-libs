...
public class IMUProtocol {
  ...
  public static final byte MSGID_YPR_UPDATE = 'y';
  ...
  // Quaternion Data Update Packet - e.g.,
  // !r[q1][q2][q3][q4][accelx][accely][accelz][magx][magy][magz]
  public static final byte MSGID_QUATERNION_UPDATE = 'q';
  ...
  // Gyro/Raw Data Update packet - e.g.,
  // !g[gx][gy][gz][accelx][accely][accelz][magx][magy][magz][temp_c][cr][lf]

  public static final byte MSGID_GYRO_UPDATE = 'g';
  ...
  public static final byte MSGID_STREAM_CMD = 'S';
  public static final int STREAM_CMD_STREAM_TYPE_YPR = MSGID_YPR_UPDATE;
  public static final int STREAM_CMD_STREAM_TYPE_QUATERNION = MSGID_QUATERNION_UPDATE;
  public static final int STREAM_CMD_STREAM_TYPE_GYRO = MSGID_GYRO_UPDATE;
  ...
  public static final byte MSG_ID_STREAM_RESPONSE = 's';
  ...
  public static final byte STREAM_MSG_TERMINATION_CHAR = (byte) '\n';

  public static final short NAV6_FLAG_MASK_CALIBRATION_STATE = 0x03;

  public static final short NAV6_CALIBRATION_STATE_WAIT = 0x00;
  public static final short NAV6_CALIBRATION_STATE_ACCUMULATE = 0x01;
  public static final short NAV6_CALIBRATION_STATE_COMPLETE = 0x02;
  ...
  public static int encodeStreamCommand(
      byte[] protocol_buffer, byte stream_type, byte update_rate_hz) {
    // Header
  ...
  public static int decodeStreamResponse(byte[] buffer, int offset, int length, StreamResponse r) {
    ...
    if ((buffer[offset + 0] == PACKET_START_CHAR)
        && (buffer[offset + 1] == MSG_ID_STREAM_RESPONSE)) {
      ...
      r.gyro_fsr_dps =
          decodeProtocolUint16(buffer, offset + STREAM_RESPONSE_GYRO_FULL_SCALE_DPS_RANGE);
      r.accel_fsr_g =
          decodeProtocolUint16(buffer, offset + STREAM_RESPONSE_ACCEL_FULL_SCALE_G_RANGE);
      r.update_rate_hz = decodeProtocolUint16(buffer, offset + STREAM_RESPONSE_UPDATE_RATE_HZ);
      r.yaw_offset_degrees =
          decodeProtocolFloat(buffer, offset + STREAM_RESPONSE_YAW_OFFSET_DEGREES);
  ...
  public static int decodeQuaternionUpdate(
      byte[] buffer, int offset, int length, QuaternionUpdate u) {
    if (length < QUATERNION_UPDATE_MESSAGE_LENGTH) {
      return 0;
    }
    if ((buffer[offset + 0] == PACKET_START_CHAR)
        && (buffer[offset + 1] == MSGID_QUATERNION_UPDATE)) {
  ...
  public static int decodeGyroUpdate(byte[] buffer, int offset, int length, GyroUpdate u) {
    ...
    if ((buffer[offset + 0] == PACKET_START_CHAR) && (buffer[offset + 1] == MSGID_GYRO_UPDATE)) {
      ...
      u.temp_c =
          decodeProtocolUnsignedHundredthsFloat(buffer, offset + GYRO_UPDATE_TEMP_VALUE_INDEX);
  ...
  public static short decodeProtocolUint16(byte[] uint16_string, int offset) {
    ...
    for (int i = offset + 0; i < offset + 4; i++) {
      byte digit =
          (byte)
              (uint16_string[i] <= '9' ? uint16_string[i] - '0' : ((uint16_string[i] - 'A') + 10));
  ...
  /* 0 to 655.35 */
  public static float decodeProtocolUnsignedHundredthsFloat(
      byte[] uint8_unsigned_hundredths_float, int offset) {
